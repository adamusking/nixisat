...
void setState(FlightState newState) {
    if (newState == myCtx.state) return;
    LOG("[STATE] %s -> %s @ %lu ms",
        stateName(myCtx.state), stateName(newState), millis());
    myCtx.state = newState;
    myCtx.state_entered_ms = millis();
}
...
void serviceReleasePin() {
    if (myCtx.released &&
        (millis() - myCtx.deploy_ms > RELEASE_HOLD_MS)) {
        digitalWrite(PIN_RELEASE, RELEASE_ACTIVE_HIGH ? LOW : HIGH);
    }
}
...
NavResult computeNavigation() {
    ...
    double lat2 = myCtx.target_lat * DEG_TO_RAD;
    double dLat = (myCtx.target_lat - gpsState.lat) * DEG_TO_RAD;
    double dLon = (myCtx.target_lon - gpsState.lon) * DEG_TO_RAD;
...
void runStabilizationAndNavigation() {
    ...
    if (myCtx.state == STATE_NAVIGATING && gpsState.valid) {
        NavResult nav = computeNavigation();
        myCtx.target_reached = (nav.distance_m < TARGET_REACHED_RADIUS_M);

        if (!myCtx.target_reached) {
...
void runFlightStateMachine() {
    ...
    case STATE_DEPLOYING:
        ...
        if (millis() - myCtx.deploy_ms >= POST_RELEASE_DELAY_MS) {
    ...
    case STATE_STABILIZING:
        ...
        if (millis() - myCtx.state_entered_ms > 1000 && gpsState.valid) {
    ...
    if (myCtx.flight_start_ms > 0 &&
        (millis() - myCtx.flight_start_ms) > MAX_FLIGHT_DURATION_MS &&
        myCtx.state != STATE_LANDED) {
...
uint16_t buildStatusFlags() {
    ...
    if (myCtx.released)            f |= FLAG_RELEASED;
    if (myCtx.state >= STATE_DEPLOYING && myCtx.state <= STATE_LANDING)
                                 f |= FLAG_MOTORS_ARMED;
    if (fabsf(att.roll) > 60.0f || fabsf(att.pitch) > 60.0f)
                                 f |= FLAG_TILT_WARN;
    if (myCtx.target_reached)      f |= FLAG_TARGET_REACHED;
    if (myCtx.state == STATE_LANDED) f |= FLAG_LANDED;
#if CALIBRATION_MODE
    f |= FLAG_CALIBRATION;
#endif
    return f;
...
void buildTelemetryPacket(TelemetryPacket& p) {
    ...
    p.flight_state = (uint8_t)myCtx.state;
    ...
    if (myCtx.loop_count > 0) {
        uint32_t mean = myCtx.loop_time_us_sum / myCtx.loop_count;
        p.loop_time_us = (uint8_t)constrain((int)(mean / 4), 0, 255);
        myCtx.loop_count = 0;
        myCtx.loop_time_us_sum = 0;
    }
}
...
void sendTelemetry() {
    ...
    LOG("[TX %lu] st=%s alt=%.1f vs=%.2f R=%.1f P=%.1f Y=%.1f gps=%d sats=%d ff=%d",
        (unsigned long)p.packetID, stateName(myCtx.state),
        alt.relative_alt_m, alt.vertical_speed,
...
void loop() {
    ...
    // 6. Periodic outputs
    uint32_t now = millis();
    uint32_t telem_interval = (myCtx.state == STATE_ARMED_PRE_LAUNCH ||
                               myCtx.state == STATE_LANDED)
                              ? TELEM_TX_INTERVAL_IDLE_MS
                              : TELEM_TX_INTERVAL_MS;
    if (now - myCtx.last_telem_tx_ms >= telem_interval) {
        myCtx.last_telem_tx_ms = now;
        sendTelemetry();
        sendLinkUpdate();
    }
    ...
    myCtx.loop_time_us_sum += loop_us;
    myCtx.loop_count++;
}
